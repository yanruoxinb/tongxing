...
function jsonToSelect(url, target, json, valueName, textName, async,isMultiple) {
    ...
    }, "POST", json, async == false ? false : true);
}
...
function common_pagination(container, totalCount, pageSize, pageindex, setMax, onCreateTable) {
    ...
    if (totalCount > 0) {
        ...
        var pageStyle = function (href, pageIndex, isCurrentPage) {
	        ...
	        pageClick(li.clone(), 1).append(a.clone().append("<<")).appendTo($(ul));
        prevOrNext(li.clone(), true).append(a.clone().append("<")).appendTo($(ul));
        ...
        prevOrNext(li.clone(), false).append(a.clone().append(">")).appendTo($(ul));
        pageClick(li.clone(), count).append(a.clone().append(">>")).appendTo($(ul));

      //  li.clone().append(a.clone().append( pageindex + "/" + count)).appendTo($(ul));
        ...
        var skipLink = a.clone().addClass("btn").css({"float": "right"}).append("跳转");
